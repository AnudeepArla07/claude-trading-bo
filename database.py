...
class TradeDatabase:
    ...
    def log_options_trade(self, decision, order):
        with self._conn() as conn:
            ...
            """, (
                ...
                decision.get("max_profit") if isinstance(decision.get("max_profit"), (int, float)) else 0,
    ...
    def print_summary(self):
        with self._conn() as conn:
            ...
            opt_trades     = conn.execute("SELECT COUNT(*) FROM options_trades").fetchone()[0]
            blocked        = conn.execute("SELECT COUNT(*) FROM decisions WHERE status='BLOCKED'").fetchone()[0]
