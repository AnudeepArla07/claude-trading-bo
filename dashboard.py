...
def run():
    while True:
        ...
        print("\n📋 Recent Decisions:")
        print(
            f"  {'Time':<20} {'Act':<5} {'Ticker':<6} {'Conf':<6} {'Status':<12} Reason"
        )
        print("  " + "─" * 85)
        for ts, action, ticker, qty, conf, status, reason in decisions:
            ...
            r_str = (reason or "")[:50]
            print(
                f"  {(ts or '')[:19]:<20} {a_icon}{(action or ''):<4} "
        ...
        if stocks:
            for ts, sym, side, qty, ostatus in stocks:
                ...
                print(
                    f"  {(ts or '')[:19]}  {icon}{side:<5} {sym:<6} x{qty}  {ostatus}"
                )
        else:
            print("  None yet.")
