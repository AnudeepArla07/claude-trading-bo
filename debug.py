...
from backtest import download_ticker, calc_rsi, calc_macd, calc_atr, calc_ema, calc_market_regime, calc_volume_ratio
...
print(f"  + RSI bouncing    : {(rsi_low & regime_bull & above_ema21 & rsi_bounce).sum()} days")
print(f"  + price bouncing  : {(rsi_low & regime_bull & above_ema21 & rsi_bounce & price_bounce).sum()} days")
print(f"  + hist improving  : {(rsi_low & regime_bull & above_ema21 & rsi_bounce & price_bounce & hist_improve).sum()} days")
print(f"  ALL conditions    : {(rsi_low & regime_bull & above_ema21 & rsi_bounce & price_bounce & hist_improve & vol_ok).sum()} days")