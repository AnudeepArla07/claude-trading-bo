...
class MarketDataFeed:
    ...
    def get_quotes(self, tickers: List[str]) -> Dict[str, dict]:
        ...
        for ticker in tickers:
            ...
            try:
                ...
                prev_close = (
                    float(snap.prev_daily_bar.c) if snap.prev_daily_bar else price
                )
                ...
                bars_5m = self._get_bars(
                    ticker, TimeFrame.Minute, limit=75, days_back=1
                )
                ...
                if len(closes_1d) >= 20:
                    ...
                    ind["sma_50"] = (
                        self._sma(closes_1d, 50) if len(closes_1d) >= 50 else None
                    )
                    ind["sma_200"] = (
                        self._sma(closes_1d, 200) if len(closes_1d) >= 200 else None
                    )
                    ...
                    log.warning(
                        "%s: only %d daily bars — indicators unavailable",
                        ticker,
                        len(closes_1d),
                    )
    ...
    def get_news(self, tickers: List[str]) -> List[dict]:
        ...
        try:
            news = self.api.get_news(
                symbol=",".join(tickers), limit=15, include_content=False
            )
            for n in news:
                items.append(
                    {
    ...
    def _macd(self, closes: List[float]) -> Optional[dict]:
        ...
        return {
            "macd": round(line, 3),
            "signal": round(line * 0.9, 3),
            "histogram": round(line * 0.1, 3),
            "bullish": line > 0,
        }

    def _bollinger(self, closes: List[float], period: int = 20) -> Optional[dict]:
        if len(closes) < period:
            return None
    ...
    def _vol_ratio(self, volumes: List[float], period: int = 20) -> Optional[float]:
        if len(volumes) < period + 1:
            return None
        avg = sum(volumes[-period - 1 : -1]) / period
        return round(volumes[-1] / avg, 2) if avg else 1.0

    def _momentum(self, closes: List[float], period: int) -> Optional[float]:
        if len(closes) < period + 1:
            return None
    ...
    def _signals(self, price: float, ind: dict) -> dict:
        ...
        if rsi:
            if rsi < 35:
                buys.append(f"RSI oversold ({rsi})")
            elif rsi < 45:
                buys.append(f"RSI low ({rsi})")
            if rsi > 70:
                sells.append(f"RSI overbought ({rsi})")
            elif rsi > 60:
                sells.append(f"RSI elevated ({rsi})")

        if rsi5m:
            if rsi5m < 30:
                buys.append(f"5m RSI oversold ({rsi5m})")
            if rsi5m > 75:
                sells.append(f"5m RSI overbought ({rsi5m})")
        ...
        if pct_b is not None:
            if pct_b < 0.1:
                buys.append(f"Lower BB ({pct_b:.2f})")
            elif pct_b > 0.9:
                sells.append(f"Upper BB ({pct_b:.2f})")
        ...
        if mom5d > 5:
            buys.append(f"5d mom +{mom5d:.1f}%")
        elif mom5d < -5:
            sells.append(f"5d mom {mom5d:.1f}%")
