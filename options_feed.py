...
class OptionsFeed:
    ...
    def get_options_data(
        self, ticker: str, stock_price: float, signal_bias: str = "NEUTRAL"
    ) -> Optional[dict]:
        try:
    ...
    # ─────────────────────────────────────────────────────────────────────────
    def _fetch_chain(
        self, ticker: str, expiry: str, stock_price: float
    ) -> Optional[dict]:
        try:
    ...
    def _select_contracts(
        self, ticker: str, price: float, chains: Dict[str, dict], bias: str
    ) -> List[dict]:
        recs = []
        for expiry, chain in chains.items():
            ...
            if bias in ("STRONG_BUY", "BUY"):
                ...
                if spread:
                    ...
                    mp = round(
                        (spread["short"]["strike"] - spread["long"]["strike"] - debit)
                        * 100
                        * qty,
                        2,
                    )
    ...
    def _best_contract(
        self, contracts: List[dict], target_delta: float
    ) -> Optional[dict]:
        valid = [
            c
            for c in contracts
    ...
    def _find_spread(
        self, contracts: List[dict], price: float, spread_type: str
    ) -> Optional[dict]:
    ...
    def _size_contracts(self, contract: dict, win_rate: float = 0.55) -> int:
        mid = contract["mid"]
        if mid <= 0:
            return 1
        avg_win = mid * 2.0
    ...
    def _compute_iv_rank(self, ticker: str, chains: Dict[str, dict]) -> Optional[float]:
        ivs = [
            c["iv"]
    ...
    def _summarize_chains(self, chains: Dict[str, dict]) -> dict:
        summary = {}
        for expiry, chain in chains.items():
            calls = sorted(chain["calls"], key=lambda c: abs(c["delta"] - 0.40))[:5]
            puts = sorted(chain["puts"], key=lambda c: abs(abs(c["delta"]) - 0.40))[:5]
            summary[expiry] = {
                "calls": [
                    {
                        "strike": c["strike"],
