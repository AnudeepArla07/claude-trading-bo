...
DEFAULT_WATCHLIST = [
    "NVDA", "AMD",  "TSLA", "AAPL", "GOOGL",
    "MSFT", "QQQ",  "PLTR", "MSTR", "COIN", "UBER",
...
def download_ticker(ticker: str, start: str,
    ...
    try:
        ...
        for c in raw.columns:
            n = str(c).strip().lower()
            if   "open"  in n: col_map[c] = "Open"
            elif "high"  in n: col_map[c] = "High"
            elif "low"   in n: col_map[c] = "Low"
            elif "close" in n: col_map[c] = "Close"
            elif "vol"   in n: col_map[c] = "Volume"
...
def calc_bollinger_pctb(closes: pd.Series,
...
def run_backtest(
    ...
    for ticker in tickers:
        ...
        all_data[ticker] = df
        print(f"  {status} {ticker:<6}: {len(df)} bars  "
    ...
    if total_entries == 0:
        ...
        print(all_data[ticker][
            ["rsi", "regime", "signal_count", "entry"]
        ].tail(10).to_string())
        return {}

    all_dates = sorted(set.union(*[
        set(df.index) for df in all_data.values()
    ]))
    ...
    for ticker in list(port.positions.keys()):
        price = (
            ...
            if last_date in all_data[ticker].index else 0
        )
...
def analyze_results(results: dict) -> dict:
    ...
    annual_return = (
        ((final_eq / init_cash) ** (1 / n_years) - 1) * 100
        if n_years > 0 else 0
    )
...
def print_report(results: dict, metrics: dict):
    ...
    print(f"  Winners          : {m['winners']:>8}  ({wr:.1f}%)  "
    ...
    print(f"  Win / Loss R:R   : {rr:>8.1f}:1  "
          f"{'✅' if rr > 1.5 else '❌'}")
    ...
    print(f"  {'─'*60}")
    print(f"  {'Ticker':<8} {'Trades':>6} {'Win%':>6} "
    ...
    if sh > 1.5:   positives.append(f"✅ Strong Sharpe ({sh:.2f})")
    elif sh > 1.0: positives.append(f"⚠️  Acceptable Sharpe ({sh:.2f})")
    else:          issues.append(f"❌ Weak Sharpe ({sh:.2f})")

    if dd > -20:   positives.append(f"✅ Drawdown manageable ({dd:.1f}%)")
    elif dd > -30: issues.append(f"⚠️  High drawdown ({dd:.1f}%)")
    else:          issues.append(f"❌ Drawdown too high ({dd:.1f}%)")

    if wr > 55:    positives.append(f"✅ Good win rate ({wr:.1f}%)")
    elif wr > 45:  positives.append(f"⚠️  Acceptable win rate ({wr:.1f}%)")
    else:          issues.append(f"❌ Low win rate ({wr:.1f}%)")

    if pf > 1.5:   positives.append(f"✅ Strong profit factor ({pf:.2f})")
    elif pf > 1.0: issues.append(f"⚠️  Weak profit factor ({pf:.2f})")
    else:          issues.append(f"❌ Losing overall (PF={pf:.2f})")

    if ar > 20:    positives.append(f"✅ Strong annual return ({ar:.1f}%)")
    elif ar > 10:  positives.append(f"⚠️  Moderate return ({ar:.1f}%)")
    else:          issues.append(f"❌ Low return ({ar:.1f}%)")
...
def save_charts(results: dict, metrics: dict) -> str:
    ...
    ax1.axhline(init, color=GR, linestyle="--",
                linewidth=0.8, alpha=0.7, label="Start")
    ...
    if results["trades"]:
        ...
        monthly = df_t.groupby(
            df_t["exit_dt"].dt.to_period("M")
        )["pl"].sum()
    ...
    if results["trades"]:
        ...
        if len(wins):
            ax4.hist(wins, bins=25, color=G, alpha=0.7,
                     label=f"Winners ({len(wins)})")
        if len(loss):
            ax4.hist(loss, bins=25, color=R, alpha=0.7,
                     label=f"Losers ({len(loss)})")
    ...
    ax5.barh(ts.index, ts["total_pl"].values,
...
def run_regime_breakdown(results: dict):
    ...
    print(f"{'═'*65}")
    print(f"  {'Period':<30} {'Return':>8} {'Sharpe':>7} "
          f"{'MaxDD':>7} {'Trades':>7}")
    print(f"  {'─'*63}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Claude Trading Bot — Backtester v8"
    )
    parser.add_argument("--ticker",  type=str,
                        help="Single ticker e.g. NVDA")
    ...
    parser.add_argument("--quick",   action="store_true",
                        help="2024 only (fast test)")
    parser.add_argument("--quiet",   action="store_true",
                        help="No individual trade logs")
    parser.add_argument("--regimes", action="store_true",
                        help="Show performance by market regime")
